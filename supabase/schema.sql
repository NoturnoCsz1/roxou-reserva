-- Reserva Roxou — Schema de referência
-- Já aplicado via Lovable Cloud. Use este arquivo para reproduzir em outra instância Supabase.

create type public.app_role as enum ('admin', 'passenger');
create type public.driver_state as enum ('online', 'busy', 'offline');
create type public.ride_status as enum (
  'pending','approved','rejected','paid','confirmed','in_progress','completed','cancelled'
);
...
create policy "driver_status read" on public.driver_status for select to anon, authenticated using (true);
...
create policy "settings read" on public.ride_settings for select to anon, authenticated using (true);
create policy "settings admin update" on public.ride_settings
  for update to authenticated using (public.has_role(auth.uid(),'admin'));
...
create policy "rides passenger" on public.ride_requests for all to authenticated
  using (passenger_id=auth.uid()) with check (passenger_id=auth.uid());
create policy "rides admin select" on public.ride_requests for select to authenticated
  using (public.has_role(auth.uid(),'admin'));
create policy "rides admin update" on public.ride_requests for update to authenticated
  using (public.has_role(auth.uid(),'admin'));
...
create policy "msgs passenger" on public.ride_messages for select to authenticated using (
  exists (select 1 from public.ride_requests r where r.id=ride_id and r.passenger_id=auth.uid())
);
create policy "msgs admin select" on public.ride_messages for select to authenticated using (public.has_role(auth.uid(),'admin'));
create policy "msgs passenger insert" on public.ride_messages for insert to authenticated with check (
  sender_id=auth.uid() and exists (select 1 from public.ride_requests r where r.id=ride_id and r.passenger_id=auth.uid())
);
create policy "msgs admin insert" on public.ride_messages for insert to authenticated with check (
